...
public class ESSFootball {
    ...
    private static class Equipo implements Comparable<Equipo> {
        ...
        public Equipo(String nombre) {
            this.nombre = nombre;
            puntos = partidos = victorias = empates = derrotas = golesFavor = golesContra = 0;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (nTorneos-- > 0) {
            ...
            for (int i = 0; i < equipos.length; i++) {
                ...
                System.out.println((i + 1) + ") " + aux.nombre + " " + aux.puntos + "p, " + aux.partidos
                        + "g (" + aux.victorias + "-"
                        + aux.empates + "-" + aux.derrotas + "), "
                        + aux.diferenciaGoles() + "gd (" + aux.golesFavor + "-" + aux.golesContra + ")");
            }
            if(nTorneos > 0)
                System.out.println();
    ...
    public static void actualizaEquipo(String nombreEquipo, int golesFavor, int golesContra, Equipo[] equipo) {
